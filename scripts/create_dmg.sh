...
# Uso: scripts/create_dmg.sh
# Requer que dist/iMackPeek.app já exista (build de Release).
...
if [[ ! -d "$APP_PATH" ]]; then
    echo "erro: $APP_PATH não existe — rode o build de Release primeiro (scripts/build_release.sh)." >&2
    exit 1
fi
...
osascript <<EOF || echo "  (osascript falhou — DMG funciona, mas sem layout custom)"
